...
public class FileUploadService implements UploadService {
        ...
        try {
            ...
            if (paths != null) {
                for (Map.Entry<String, PathItem> pathEntry : paths.entrySet()) {
                    ...
                    if (pathValue.getGet() != null && processOperation(pathName, pathValue.getGet(), "GET", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getPost() != null && processOperation(pathName, pathValue.getPost(), "POST", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getPut() != null && processOperation(pathName, pathValue.getPut(), "PUT", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getDelete() != null && processOperation(pathName, pathValue.getDelete(), "DELETE", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getOptions() != null && processOperation(pathName, pathValue.getOptions(), "OPTIONS", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getHead() != null && processOperation(pathName, pathValue.getHead(), "HEAD", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                    if (pathValue.getTrace() != null && processOperation(pathName, pathValue.getTrace(), "TRACE", version, openAPI, changesDetected)) {
                        hasChanges = true; // Changement détecté
                    }
                }
            }
            ...
            // Supprimer les API obsolètes
            if (deleteObsoleteApis(currentPaths, changesDetected)) {
                hasChanges = true; // Changement détecté
            }
    ...
    private void saveApiChange(String changement, String donneeChange) {
        ...
        if (existingChanges.isEmpty()) {
            ...
            apiChange.setChangement(changement);
            apiChange.setTemps(LocalDateTime.now());
            apiChange.setDonneeChange(donneeChange);
            apiChangeRepository.save(apiChange);
        } else {
            // Si la table n'est pas vide, met à jour le dernier enregistrement
            ApiChange apiChangeToUpdate = existingChanges.get(0); // On prend le dernier, pas forcement l'id = 1
            apiChangeToUpdate.setChangement(changement);
            apiChangeToUpdate.setTemps(LocalDateTime.now());
            apiChangeToUpdate.setDonneeChange(donneeChange);
            apiChangeRepository.save(apiChangeToUpdate);
        }
    }
    ...
    /*commentaire*/
    /*private boolean deleteObsoleteApis(Set<String> currentPaths, List<String> changesDetected) {
        boolean hasChanged = false;
        List<Api> existingApis = apiRepository.findAll();
        ...
        for (Api existingApi : existingApis) {
            if (!currentPaths.contains(existingApi.getPath())) {
                ...
                List<ApiParameters> parametersReferencingApi = apiParametersRepository.findByApi(existingApi);
                if (!parametersReferencingApi.isEmpty()) {
                    String message = "Suppression des paramètres référençant l'API obsolète: " + existingApi.getPath();
                    logChanges(message);
                    apiParametersRepository.deleteAll(parametersReferencingApi);
                    changesDetected.add(message);
                }
                ...
                List<com.example.api_tierces.model.ApiResponse> responsesReferencingApi = apiResponseRepository.findByApi(existingApi);
                if (!responsesReferencingApi.isEmpty()) {
                    String message = "Suppression des réponses référençant l'API obsolète: " + existingApi.getPath();
                    logChanges(message);
                    apiResponseRepository.deleteAll(responsesReferencingApi);
                    changesDetected.add(message);
                }
                ...
                // 3. Supprimer l'API elle-même
                String message = "Suppression de l'API obsolète: " + existingApi.getPath() + " (" + existingApi.getMethod() + ")";
                logChanges(message);
                try {
                    apiRepository.delete(existingApi);
                    changesDetected.add(message);
...
/*    private boolean deleteObsoleteApis(Set<String> currentPathsAndMethods, List<String> changesDetected) {
        boolean hasChanged = false;
        List<Api> existingApis = apiRepository.findAll();
        ...
        for (Api existingApi : existingApis) {
            ...
            if (currentPathsAndMethods.contains(apiIdentifier)) {
